 ...
 *
 * I numerini sul mockup sono `aria-hidden`: il significato lo porta la lista
    ...
    <section className="border-t border-border py-20 md:py-28">
      <div className="container-narte">
        <div className="max-w-2xl">
          <Reveal>
            <p className="accent-label mb-3">cosa ottieni</p>
          </Reveal>
          <Reveal delay={0.1}>
            <h2 className="display-xl text-balance text-4xl md:text-6xl">
              Una pagina che parla al posto tuo.
            </h2>
          </Reveal>
          <Reveal delay={0.2}>
            <p className="mt-5 text-pretty text-base text-muted-foreground md:text-lg">
              Questo è quello che vede un organizzatore quando ti trova. Un link
              solo, da mandare anche fuori da qui.
            </p>
          </Reveal>
        </div>

        <div className="mt-12 grid gap-12 md:mt-16 lg:grid-cols-[auto_1fr] lg:items-center lg:gap-16">
          {/* MOCKUP */}
          <Reveal>
          ...
          {/* ANNOTAZIONI. La colonna è limitata: a tutta larghezza le righe
              arrivavano a novanta caratteri e diventavano faticose da leggere. */}
          <div>
            <ol className="max-w-xl space-y-6">
              {PROFILE_ANNOTATIONS.map((a, i) => (
                <li key={a.title}>
                  <Reveal delay={0.1 * (i + 1)} className="flex gap-4">
                    <span className="inline-flex size-8 shrink-0 items-center justify-center rounded-full bg-accent font-display text-sm font-bold tabular-nums text-accent-foreground">
                      {i + 1}
                    </span>
                    <div>
...
/** Pallino numerato sul mockup. Decorativo: il testo è nella lista accanto. */
